function [results,spa_results] = fit_tcspc_dks_nlls...
    ...
    arguments
        ...
        args.EngineType {mustBeMember(args.EngineType,["Single",...
            "Simultaneous"])} = "Single"
    end
    ...
    %% Validate optional arguments block - Single
    % Cannot do single analysis if there are multiple curves and global
    % parameters are specified
    gflag = any(gparam);
    if args.EngineType == "Single"
        assert(~gflag && ntrans > 1)
    end
    %% Pass to subroutine
    if nargout == 1
        [results] = ...
            ...
            args.UseParallel,args.EngineType);
    else
        [results,spa_results] = ...
            fit_tcspc_dks_nlls_subroutine(tcal,ntcal,trans,ndata,ntrans,...
            irf,ntransirf,bck,ntransbck,fit_start,nfitstart,fit_end,...
